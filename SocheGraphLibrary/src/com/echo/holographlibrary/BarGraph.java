...
public class BarGraph extends View {
    ...
    public void setBars(ArrayList<Bar> points){
        ...
        mShouldUpdate = true;
        for(Bar bar : mBars){
        	bar.setStep((bar.getDesValue()-bar.getValue()) / STEP_COUNT);
        }
        curStepCount=0;
        postInvalidate();
    }
    ...
    public void onDraw(Canvas ca) {
        ...
        if (mShouldUpdate) {
            ...
            for (final Bar bar : mBars) {
                if (bar.getDesValue() > maxValue) {
                    maxValue = bar.getDesValue();
                }
            }
            ...
            for (final Bar bar : mBars) {
                ...
                int height = (int) (usableHeight*bar.getValue()/maxValue);
                int top = (int)(getHeight()-bottomPadding-height);
                ...
                bar.setValue(bar.getValue()+bar.getStep());
